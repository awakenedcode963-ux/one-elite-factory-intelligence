...
import { SupersedeEvidenceCommand } from '../commands/supersede-evidence.command.js';
import { CommandValidationError } from '../errors/command-validation.error.js';

export class SupersedeEvidenceHandler {
  constructor(
    private readonly repository: IInvestigationRepository<Investigation>,
    private readonly uow: IUnitOfWork
  ) {}

  async execute(command: SupersedeEvidenceCommand): Promise<ResultEnvelope<void>> {
    try {
      ...
      if (!command.oldEvidenceId || !command.newEvidence) throw new CommandValidationError("Missing required fields");
      ...
      aggregate.supersedeEvidence(command.oldEvidenceId, command.newEvidence);
