...
import { ApproveVerificationCommand } from '../commands/approve-verification.command.js';
import { CommandValidationError } from '../errors/command-validation.error.js';

export class ApproveVerificationHandler {
  constructor(
    private readonly repository: IInvestigationRepository<Investigation>,
    private readonly uow: IUnitOfWork
  ) {}

  async execute(command: ApproveVerificationCommand): Promise<ResultEnvelope<void>> {
    try {
      ...
      if (!command.testId || !command.authorization) throw new CommandValidationError("Missing required fields");
      ...
      aggregate.approveVerification(command.testId, command.authorization);
